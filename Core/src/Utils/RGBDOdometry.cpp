  ...
  cx(cx), cy(cy), fx(fx), fy(fy)
{
    ...
    {
        cl_int2 nextDim = {height >> i, width >> i};
        pyrDims.push_back(nextDim);
    }

    for(int i = 0; i < NUM_PYRS; i++)
    {
        lastDepth[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);
        lastImage[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);

        nextDepth[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);
        nextImage[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);

        lastNextImage[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);

        nextdIdx[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);
        nextdIdy[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);

        pointClouds[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);

        corresImg[i].create(pyrDims.at(i).s[0], pyrDims.at(i).s[1]);
    }
...
void RGBDOdometry::initICP(GPUTexture * filteredDepth, const float depthCutoff)
{
	/*
    cudaArray * textPtr;
    cudaGraphicsMapResources(1, &filteredDepth->cudaRes);//������Դ
    cudaGraphicsSubResourceGetMappedArray(&textPtr, filteredDepth->cudaRes, 0, 0);//��ȡ��Դָ��
    ...
    cudaGraphicsUnmapResources(1, &filteredDepth->cudaRes); //������Դ
	*/
 
	//test
	cl_int error;
	cl_command_queue queue = *(ocl::GetInstance()->queue);
	error = clEnqueueAcquireGLObjects(queue, 1, &filteredDepth->mem, 0, 0, NULL);//������Դ	  
	int size = depth_tmp[0].colsBytes()*depth_tmp[0].rows();
	error = clEnqueueCopyBuffer(queue, filteredDepth->mem, depth_tmp[0].getDeviceMemory(), 0, 0, size, NULL, NULL, NULL);
	error = clFinish(queue);
	error = clEnqueueReleaseGLObjects(queue, 1, &filteredDepth->mem, 0, 0, NULL);//������Դ
	ocl::GetInstance()->checkError(error);
	//end test
    ...
    for(int i = 0; i < NUM_PYRS; ++i)
    {
        CameraModel cam_model = CameraModel_level(intr,i);
        createVMap(cam_model, depth_tmp[i], vmaps_curr_[i], depthCutoff);
...
void RGBDOdometry::initICP(GPUTexture * predictedVertices, GPUTexture * predictedNormals, const float depthCutoff)
{
	...
	*/
	...
	cl_int error;
	cl_command_queue queue = *(ocl::GetInstance()->queue);
	clEnqueueAcquireGLObjects(queue, 1, &predictedVertices->mem, 0, 0, NULL);
	int size = vmaps_tmp.sizeBytes();
	error = clEnqueueCopyBuffer(queue, predictedVertices->mem, vmaps_tmp.getDeviceMemory(), 0, 0, size, NULL, NULL, NULL);
	clEnqueueReleaseGLObjects(queue, 1, &predictedVertices->mem, 0, 0, NULL);
	clEnqueueAcquireGLObjects(queue, 1, &predictedNormals->mem, 0, 0, NULL);
	size = nmaps_tmp.sizeBytes();
	error = clEnqueueCopyBuffer(queue, predictedNormals->mem, nmaps_tmp.getDeviceMemory(), 0, 0, size, NULL, NULL, NULL);
	clFinish(queue);
	clEnqueueReleaseGLObjects(queue, 1, &predictedNormals->mem, 0, 0, NULL);
	ocl::GetInstance()->checkError(error);
	//end test
                                ...
                                const Eigen::Matrix4f & modelPose)
{
	...
	*/
	...
	cl_int error;
	cl_command_queue queue = *(ocl::GetInstance()->queue);
	error = clEnqueueAcquireGLObjects(queue, 1, &predictedVertices->mem, 0, 0, NULL);
	int size = vmaps_tmp.sizeBytes();
	error = clEnqueueCopyBuffer(queue, predictedVertices->mem, vmaps_tmp.getDeviceMemory(), 0, 0, size, NULL, NULL, NULL); 
	error = clEnqueueReleaseGLObjects(queue, 1, &predictedVertices->mem, 0, 0, NULL);
 	error = clEnqueueAcquireGLObjects(queue, 1, &predictedNormals->mem, 0, 0, NULL); 
	size = nmaps_tmp.sizeBytes();
	error = clEnqueueCopyBuffer(queue, predictedNormals->mem, nmaps_tmp.getDeviceMemory(), 0, 0, size, NULL, NULL, NULL); 
	error = clFinish(queue);
	error = clEnqueueReleaseGLObjects(queue, 1, &predictedNormals->mem, 0, 0, NULL);
	ocl::GetInstance()->checkError(error);
	//end test
    ...
    mat33 device_Rcam = Eigen2Mat33(Rcam);
    cl_float3 device_tcam = *reinterpret_cast<cl_float3*>(tcam.data());
                                    ...
                                    DeviceArray2D<unsigned char> * destImages)
{
	...
	cl_int error;
	cl_command_queue queue = *(ocl::GetInstance()->queue);
	error = clEnqueueAcquireGLObjects(queue, 1, &rgb->mem, 0, 0, NULL); 	 
	imageBGRToIntensity(rgb->mem,rgb->texture->width,rgb->texture->height, destImages[0]);
	error = clFinish(queue); 
	error = clEnqueueReleaseGLObjects(queue, 1, &rgb->mem, 0, 0, NULL);
	ocl::GetInstance()->checkError(error);
	//end test
...
void RGBDOdometry::initFirstRGB(GPUTexture * rgb)
{
	...
	cl_command_queue queue = *(ocl::GetInstance()->queue);
	error = clEnqueueAcquireGLObjects(queue, 1, &rgb->mem, 0, 0, NULL);
	imageBGRToIntensity(rgb->mem, rgb->texture->width, rgb->texture->height,lastNextImage[0]);
	error = clFinish(queue);	
	error = clEnqueueReleaseGLObjects(queue, 1, &rgb->mem, 0, 0, NULL);
	ocl::GetInstance()->checkError(error);
	//end test
                                                ...
                                                const bool & so3)
{
    ...
    if(so3)
    {
        ...
        K(0, 0) = CameraModel_level(intr,pyramidLevel).fx;
        K(1, 1) = CameraModel_level(intr,pyramidLevel).fy;
        K(0, 2) = CameraModel_level(intr,pyramidLevel).cx;
        K(1, 2) = CameraModel_level(intr,pyramidLevel).cy;
        K(2, 2) = 1;
    ...
    mat33 device_Rprev_inv = Eigen2Mat33(Rprev_inv);
    cl_float3 device_tprev = *reinterpret_cast<cl_float3*>(tprev.data());
    ...
    for(int i = NUM_PYRS - 1; i >= 0; i--)
    {
        ...
        K(0, 0) = CameraModel_level(intr,i).fx;
        K(1, 1) = CameraModel_level(intr,i).fy;
        K(0, 2) = CameraModel_level(intr,i).cx;
        K(1, 2) = CameraModel_level(intr,i).cy;
        K(2, 2) = 1;
        ...
        for(int j = 0; j < iterations[i]; j++)
        {
            ...
            Kt = K * Kt;
            cl_float3 kt = {(float)Kt(0), (float)Kt(1), (float)Kt(2)};
            ...
            mat33 device_Rcurr = Eigen2Mat33(Rcurr);
            cl_float3 device_tcurr = *reinterpret_cast<cl_float3*>(tcurr.data());
            ...
            {
                TICK("icpStep");
               CameraModel cm_model = CameraModel_level(intr,i);
            ...
            if(rgb)
            {
                ...
                rgbStep(corresImg[i],
                        sigmaVal,
                        pointClouds[i],
                        CameraModel_level(intr,i).fx,
                        CameraModel_level(intr,i).fy,
                        nextdIdx[i],
                        nextdIdy[i],
                        sobelScale,
                        sumDataSE3,
